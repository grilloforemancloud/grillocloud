...
if (isset($_POST["submit_button"])) {
    ...
    if (empty($nomeBase) || $host_portA <= 0 || $host_portB <= 0) {
        ...
        if ($httpCode == 201) {
            ...
            $containerId = $result->Id; // ID real do Docker
            ...
            if (empty($_POST["container_id"])) {
                ...
                $sql = "INSERT INTO container (container_id, nome, portA, portB, timeInput, timeEnd, user_id) 
                        VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssiissi", $containerId, $containerName, $host_portA, $host_portB, $timeInput, $timeEnd, $userId);
                $stmt->execute();
                $stmt->close();
            }
        } else {
