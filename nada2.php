...
$dockerApiUrl = 'http://localhost:2375/containers';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['stop_container_id'])) {
        ...
        $stopUrl = $dockerApiUrl . '/' . $containerId . '/stop';
        ...
        $update = $conn->prepare("UPDATE container SET timeEnd=? WHERE container_id=? AND user_id=?");
        $update->bind_param("ssi", $timeEnd, $containerId, $_SESSION['user_id']);
        $update->execute();
        $update->close();
    ...
    // START
    if (isset($_POST['start_container_id'])) {
        $containerId = $_POST['start_container_id'];

        $startUrl = $dockerApiUrl . '/' . $containerId . '/start';
        ...
        $reset = $conn->prepare("UPDATE container SET timeEnd='0000-00-00' WHERE container_id=? AND user_id=?");
        $reset->bind_param("si", $containerId, $_SESSION['user_id']);
        $reset->execute();
        $reset->close();
    ...
    if (isset($_POST['delete_container_id'])) {
        ...
        // Remove do Docker
        $deleteUrl = $dockerApiUrl . '/' . $containerId . '?force=true';
        ...
        if ($code == 204) {
            $del = $conn->prepare("DELETE FROM container WHERE container_id=? AND user_id=?");
            $del->bind_param("si", $containerId, $_SESSION['user_id']);
            $del->execute();
            $del->close();
        } else {
...
$sql = "SELECT id, container_id, nome, portA, portB, timeInput, timeEnd
        FROM container
        WHERE user_id = ?";
        ...
        <table>
            ...
            <tbody>
                <?php foreach ($containers as $c): ?>
                    <tr>
                        <td><?php echo $c['id']; ?></td>
                        <td><?php echo substr($c['container_id'],0,12); ?></td>
                        ...
                        <td>
                            <form method="POST" style="display:inline-block;">
                                <input type="hidden" name="stop_container_id" value="<?php echo $c['container_id']; ?>">
                                <button type="submit" class="btn-action btn-stop">Parar</button>
                            </form>
                            <form method="POST" style="display:inline-block;">
                                <input type="hidden" name="start_container_id" value="<?php echo $c['container_id']; ?>">
                                <button type="submit" class="btn-action btn-start">Startar</button>
                            </form>
                            <form method="POST" style="display:inline-block;">
                                <input type="hidden" name="delete_container_id" value="<?php echo $c['container_id']; ?>">
